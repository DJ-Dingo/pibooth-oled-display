...
from luma.oled.device import ssd1306, ssd1309, ssd1322, ssd1325, ssd1327, ssd1331, ssd1351, ssd1362, sh1106, ws0010
...
def pibooth_configure(cfg):
    ...
    cfg.add_option('OLED_I2C', 'oled_devices', "ssd1306",
                   ...
                   "Choose OLED device", ["ssd1306", "ssd1309", "ssd1322", "ssd1325", "ssd1327", "ssd1331", "ssd1351", "ssd1362", "sh1106", "ws0010"])
    ...
    cfg.add_option('OLED_I2C', 'oled_width', "128",
                   ...
                   "OLED screen width", ["16", "32", "64", "96", "128", "160", "192", "224" "256"])
    cfg.add_option('OLED_I2C', 'oled_height', "64",
                   'Change screen HEIGHT 32, 64(Default), 96, 128',
                   "OLED screen height", ["16", "32", "64", "96", "128", "160", "192", "224" "256"])
    ...
    _logos = os.listdir(logo_path)  #'/home/pi/.config/pibooth/logo/'
    matching = [s for s in _logos if _height in s]
...
def connect_oled_i2c(app, cfg):
    ...
    try:
        ...
        if "sh1106" in d:
            ...
            app.device = ws0010(app.serial, rotate=app.rotate_screen, width=app.screen_width, height=app.screen_height)
    except OSError:
        pass      
...
def write_text_to_oled(app, cfg):
    ...
    if "No" in y:
        with canvas(app.device) as app.draw: # , dither=False
        ...
        try:
            ...
            if "Yes" in y:
                if app.device.height == 32:
                    app.image = Image.open('/home/pi/.config/pibooth/logo/' + app.logos).convert('1')
                    # Display image
...
def state_chosen_do(app):
    ...
    try:
        ...
        if "Yes" in s:
            if app.device.height == 32:
                app.image = Image.open('/home/pi/.config/pibooth/oled_states/layout1_32.png').convert('1')
            elif app.device.height == 64:
                app.image = Image.open('/home/pi/.config/pibooth/oled_states/layout1_64.png').convert('1')
        # Display image
        app.device.display(app.image)
    except OSError:
        pass
